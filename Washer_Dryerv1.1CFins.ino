...
unsigned long resetWasherTime = 25000;
...
unsigned long runningWasherTime; //for running total washer time
unsigned long runningDryerTime; // for running total dryer time
...
{  
 ...
 if (washercounter == 2 || washercounter == 4 || washercounter == 6 || washercounter == 8) //increments washSample while running (not during intial fill though) for power and cost calcs 
...
void setup()
{
  ...
  Blynk.syncAll();
   ...
   bridge1.setAuthToken("yyyyyyyyyyyyyyyyyyyyyyyyyy"); // Token for Raspi3 (school bus alarms)
 }  
